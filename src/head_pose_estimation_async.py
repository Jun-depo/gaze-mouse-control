...
class Model_HeadPose:
    ...
    def __init__(self, model_name, device='CPU', extensions=None):
        ...
        # self.output_angle_y="angle_y_fc" 
        # self.output_angle_p="angle_p_fc" 
        # self.output_angle_r="angle_r_fc"      
    ...
    def predict_async(self, image, request_id=0):
        ...
        if not None:
            ...
            angle_y = self.net.requests[0].outputs["angle_y_fc"]
            angle_p = self.net.requests[0].outputs["angle_p_fc"]
            angle_r = self.net.requests[0].outputs["angle_r_fc"]

            head_pose_angles = self.preprocess_output(angle_y, angle_p, angle_r)
    ...
    def preprocess_output(self, angle_y, angle_p, angle_r):
        '''
        Before feeding the output of this model to the next model,
        you might have to preprocess the output. This function is where you can do that.
        '''
        
        if angle_y or angle_p or angle_r:

            angle_y = np.squeeze(angle_y)
            angle_p = np.squeeze(angle_p)
            angle_r = np.squeeze(angle_r)
