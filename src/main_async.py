...
from face_detection_async import Model_FaceDetection
from head_pose_estimation_async import Model_HeadPose
from facial_landmarks_detection_async import Model_FaceLandmark
from gaze_estimation_async import Model_GazeEstimation

from mouse_controller import MouseController

def main(args):
    logging.basicConfig(filename='appAsync.log', level=logging.DEBUG)
    ...
    while input_feed.cap.isOpened():    
        ...
        for img in input_feed.next_batch(): 
            ...
            boxes = face_model.predict_async(img)
            ...
            if len(boxes) > 0:
                ...
                for box in boxes:
                    ...
                    head_pose_angles = head_pose_model.predict_async(face_crop)
                    ...
                    landmarks = face_landmark_model.predict_async(face_crop)
                    ...
                    if args.display_boxes:
                        frame = cv2.rectangle(img, (x_left, y_left), (x_left+70, y_left+50), (0, 0, 125), 2)
                        frame = cv2.rectangle(img, (x_right, y_right), (x_right+70, y_right+50), (0, 0, 125), 2)


                    #print("gaze_model.model.inputs: ", gaze_model.model.inputs)
                    gaze_vector = gaze_model.predict_async(left_crop, right_crop, head_pose_angles)
